...
export default function DenylistPage({ profileId }: DenylistPageProps) {
  ...
  const loadDenylist = async () => {
    try {
      setLoading(true);
      const res = await fetch(`/api/profiles/${encodeURIComponent(profileId)}/denylist`);
      ...
      console.error('Error loading denylist:', err);
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleAdd = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.ok) {
        setMessage({ type: 'success', text: 'Domain added to denylist' });
        setNewDomain('');
        await loadDenylist();
      } else {
  ...
  const handleRemove = async (domain: string) => {
    if (!confirm(`Remove ${domain} from denylist?`)) return;

    try {
      setMessage(null);

      const res = await fetch(
        `/api/profiles/${encodeURIComponent(profileId)}/denylist/${encodeURIComponent(domain)}`,
        { method: 'DELETE' }
      );

      if (res.ok) {
        setMessage({ type: 'success', text: 'Domain removed from denylist' });
        await loadDenylist();
      } else {
    ...
    <div className="space-y-6">
      ...
      <div>
        ...
        <p className="text-slate-400 text-sm mt-1">
          Explicitly block domains regardless of other settings
        </p>
      </div>
      ...
      <div className="backdrop-blur-xl bg-white/5 rounded-2xl border border-slate-700/50 overflow-hidden">
        {filteredEntries?.length > 0 ? (
                ...
                {filteredEntries.map((entry) => (
                  <tr key={entry?.id} className="hover:bg-slate-800/30 transition-colors">
                    ...
                    <td className="px-6 py-4 text-sm">
                      <span className="px-2.5 py-1 rounded-full bg-red-500/10 border border-red-500/30 text-red-400 text-xs">
                        Blocked
                      </span>
                    </td>
                    <td className="px-6 py-4 text-right">
                      <button
                        ...
                        className="px-3 py-1.5 rounded-full bg-slate-700/50 hover:bg-slate-700 border border-slate-600 text-slate-300 text-xs transition-colors flex items-center gap-1 ml-auto"
                      >
          ...
          <div className="p-12 text-center text-slate-400">
            {searchQuery ? 'No domains match your search' : 'No domains in denylist yet'}
          </div>
        )}
      </div>
    </div>
